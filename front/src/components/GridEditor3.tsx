...
export default function GridEditor3({ onBinClick, onBinDoubleClick }: GridEditor3Props) {
  ...
  const validatePlacement3D = (
    ...
    const targetHeight = Math.max(0.5, heightUnits || 1);
  ...
  const handleDrop = async (_: Layout[], item: Layout, e: any) => {
    ...
    const placementCheck = validatePlacement3D(newX, newY, bin.width_units, bin.depth_units, bin.height_units || 1, bin.z_offset || 0, binId);
    if (!placementCheck.valid) {
  ...
  // Specific item change applied after drag/resize finishes
  ...
  const handleAddBin = async () => {
    ...
    for (let j = 0; j < currentDrawer.depth_units; j++) {
      for (let i = 0; i < currentDrawer.width_units; i++) {
        const checkHeight = subLayerOffset === 0.5 ? 0.5 : 1;
        const placementCheck = validatePlacement3D(i, j, 1, 1, checkHeight, subLayerOffset);
        if (placementCheck.valid) {
          x = i;
          y = j;
          foundPosition = true;
    ...
    const newBinData = {
      ...
      width_units: 1,
      depth_units: 1,
      height_units: subLayerOffset === 0.5 ? 0.5 : 1,
            ...
            >
              {() => (
                <>
                  ...
                  >
                    <div
                      className="relative bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden"
                      style={{
